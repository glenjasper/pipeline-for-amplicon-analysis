...
class Parse:
    ...
    def read_abundance_table(self, file, prefix):
        ...
        for level_taxa in arr_taxa:
            ...
            # Data for the PCA
            # -----------------------------
            df_level_pca = df_level.copy()

            # Hellinger transform
            for colname, coltype in df_level_pca.dtypes.iteritems():
                df_level_pca[colname] = df_level_pca[[colname]] ** .5

            df_level_pca['Sample'] = df_level_pca.idxmax(axis = 1)

            # Converter o Index (gerada no groupby) em coluna
            df_level_pca = df_level_pca.reset_index(level = 0)

            # Renomeando a coluna Taxon
            df_level_pca = df_level_pca.rename({level_taxa: ''}, axis = 'columns')

            output_file = '%s_data-pca-abundances_%s.txt' % (prefix, level_taxa)
            output_file = os.path.join(self.OUTPUT_PATH, output_file.lower())

            df_level_pca.to_csv(output_file, sep = '\t', encoding = 'utf-8', index = False)

            self.show_print("  [PCA] Abundance table: %s" % output_file, [self.LOG_FILE], font = self.BIGREEN)
            self.show_print("", [self.LOG_FILE])

            # -----------------------------
            # Data for the Heatmap
            # -----------------------------
            df_level_heatmap = df_level.copy()
            ...
            # Converter o Index (gerada no groupby) em coluna
            df_level_heatmap = df_level_heatmap.reset_index(level = 0)
            ...
            df_level_bar = df_level.stack().reset_index()

            # Renomeando as colunas
            df_level_bar.columns = [level_taxa, 'sample', 'value']

            output_file = '%s_data-bar-abundances_%s.txt' % (prefix, level_taxa)
            output_file = os.path.join(self.OUTPUT_PATH, output_file.lower())

            df_level_bar.to_csv(output_file, sep = '\t', encoding = 'utf-8', index = False)
